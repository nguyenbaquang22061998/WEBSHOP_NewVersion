...
namespace DOLPHIN.Service.Common
{
    ...
    public abstract class EntityService<T> : IEntityService<T>
    {
        ...
        /// <inheritdoc/>
        public virtual async Task<bool> ValidatePermission(Guid userId, long[] permissionIds, IRolePermissionRepository rolePermissionRepo)
        {
            var permissions = (await rolePermissionRepo.GetAll()).Where(rp => rp.Roles.RoleUsers.Any(ru => ru.UserId == userId));

            return permissions.Any(p => permissionIds.Contains(p.PermissionId));
        }
    }
}
